...
class MainActivity : Activity() {
    ...
    private fun buildUi() {
        ...
        modeLabel = TextView(this).apply { textSize = 16f; setPadding(0, 8, 0, 24) }
        ...
        content.addView(makeButton("Notification rules") { startActivity(Intent(this, NotificationRulesActivity::class.java)) })
        pairingLabel = TextView(this).apply { textSize = 17f; setPadding(0, 28, 0, 8) }
        content.addView(pairingLabel)
        ipInput = EditText(this).apply { hint = "Parent IP address"; maxLines = 1; inputType = android.text.InputType.TYPE_CLASS_TEXT }
        codeInput = EditText(this).apply { hint = "6-digit pairing code"; maxLines = 1; inputType = android.text.InputType.TYPE_CLASS_NUMBER }
        content.addView(ipInput); content.addView(codeInput)
        content.addView(makeButton("Start") { startSelectedMode() })
        content.addView(makeButton("Stop") { stopMonitoring() })
        statusLabel = TextView(this).apply { textSize = 16f; setPadding(0, 24, 0, 16) }
        statsLabel = TextView(this).apply { textSize = 22f; setTypeface(null, Typeface.BOLD); gravity = Gravity.CENTER_HORIZONTAL; setPadding(0, 16, 0, 24) }
        content.addView(statusLabel); content.addView(statsLabel)
    }

    private fun showLocalMode() {
        localOnly = true; stopMonitoringServiceOnly()
        ...
        statusLabel.text = "Local monitoring active"; startLocalPolling()
    }

    private fun showParentMode() {
        localOnly = false; stopLocalPolling()
        ...
        ipInput.visibility = View.GONE; codeInput.visibility = View.GONE
        modeLabel.text = "Receives battery, CPU, RAM and time-to-empty from a paired child or Termux sender."
        statusLabel.text = "Ready to listen"; statsLabel.text = "No child connected"
    }
    ...
    private fun startSelectedMode() {
        if (localOnly) { startLocalPolling(); return }
        if (ipInput.visibility == View.VISIBLE) {
            ...
            startMonitorService(Intent(this, MonitorService::class.java).apply {
                action = MonitorService.ACTION_START_CHILD; putExtra(MonitorService.EXTRA_HOST, host); putExtra(MonitorService.EXTRA_CODE, code)
            })
        } else {
            startMonitorService(Intent(this, MonitorService::class.java).apply {
                action = MonitorService.ACTION_START_PARENT; putExtra(MonitorService.EXTRA_CODE, getOrCreatePairingCode())
            })
        }
    }
        ...
        val task = object : Runnable {
            override fun run() {
                ...
                Thread {
                    ...
                    runOnUiThread { if (localPolling) showStats(snapshot.batteryPercent, snapshot.cpuPercent, snapshot.ramPercent, snapshot.batteryMinutesRemaining ?: -1L) }
                }.start()
    ...
    private fun getOrCreatePairingCode(): String {
        ...
        val code = (100000 + SecureRandom().nextInt(900000)).toString(); prefs.edit().putString("code", code).apply(); return code
    }
    private fun getLocalIpv4(): String? = try {
        NetworkInterface.getNetworkInterfaces().asSequence().flatMap { it.inetAddresses.asSequence() }.filterIsInstance<Inet4Address>()
    ...
    private fun registerReceiverCompat() {
        val filter = IntentFilter().apply { addAction(MonitorService.ACTION_STATUS); addAction(MonitorService.ACTION_TELEMETRY) }
