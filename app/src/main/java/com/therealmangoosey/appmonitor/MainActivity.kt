...
class MainActivity : Activity() {
    ...
    private fun buildUi() {
        ...
        content.addView(makeButton("Webhooks") { startActivity(Intent(this, WebhooksActivity::class.java)) })

        pairingLabel = TextView(this).apply { textSize = 17f; setPadding(0, 28, 0, 8) }
        content.addView(pairingLabel)
        ipInput = EditText(this).apply { hint = "Parent IP address"; maxLines = 1 }
    ...
    private fun showParentMode() {
        ...
        statusLabel.text = "Starting parent listener..."
        statsLabel.text = "No device connected"
    ...
    private fun startSelectedMode() {
        if (ipInput.visibility == View.VISIBLE) {
            ...
            startMonitorService(Intent(this, MonitorService::class.java).apply {
                action = MonitorService.ACTION_START_LOCAL
            })
        }
    }
    ...
    private fun makeButton(text: String, click: () -> Unit): Button = Button(this).apply {
        this.text = text
