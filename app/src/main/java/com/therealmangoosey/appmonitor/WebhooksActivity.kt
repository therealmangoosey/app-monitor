...
class WebhooksActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        root.addView(TextView(this).apply {
            text = "Send the telemetry from the device that actually produced it. Discord and Slack get formatted cards; Generic sends JSON."
    ...
    private fun refresh() {
        ...
        items.forEach { config ->
            ...
            row.addView(TextView(this).apply {
                text = "${config.name}\nPreset: ${platformName(config.platform)}\n${if (config.enabled) "Enabled" else "Disabled"} • ${if (config.sendEveryUpdate) "Every update" else "Manual/test only"}"
                textSize = 16f
            })
            val buttons = LinearLayout(this)
            buttons.addView(Button(this).apply { text = "Test"; setOnClickListener {
                val test = Telemetry(73, false, 18, 42, 215)
                Thread { WebhookSender.send(config.copy(sendEveryUpdate = true), test, "Test device") }.start()
            } })
            buttons.addView(Button(this).apply { text = "Edit"; setOnClickListener { showEditor(config) } })
            buttons.addView(Button(this).apply { text = if (config.enabled) "Disable" else "Enable"; setOnClickListener { store.update(config.copy(enabled = !config.enabled)); refresh() } })
            buttons.addView(Button(this).apply { text = "Delete"; setOnClickListener { store.remove(config.id); refresh() } })
            row.addView(buttons)
    ...
    private fun showEditor(existing: WebhookConfig?) {
        ...
        fun updateDetail() { detail.text = when (preset.selectedItemPosition) {
            0 -> "Discord: polished embed with fields, timestamp and source."
            1 -> "Slack: formatted Block Kit message with device statistics."
            else -> "Generic: plain JSON for services that accept custom webhook payloads."
        } }
        preset.onItemSelectedListener = object : android.widget.AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: android.widget.AdapterView<*>?, view: View?, position: Int, id: Long) = updateDetail()
            override fun onNothingSelected(parent: android.widget.AdapterView<*>?) = Unit
        }
        box.addView(TextView(this).apply { text = "Preset"; textSize = 13f })
        box.addView(preset); box.addView(detail); box.addView(name); box.addView(url); box.addView(every)
        box.addView(battery); box.addView(charging); box.addView(cpu); box.addView(ram); box.addView(time)
        existing?.let { c ->
            preset.setSelection(when (c.platform) { "discord" -> 0; "slack" -> 1; else -> 2 })
            name.setText(c.name); url.setText(c.url)
        } ?: run { name.setText("Device status"); updateDetail() }

        AlertDialog.Builder(this)
            .setTitle(if (existing == null) "New webhook" else "Edit webhook")
            .setView(box)
            .setPositiveButton("Save") { _, _ ->
                val platform = when (preset.selectedItemPosition) { 0 -> "discord"; 1 -> "slack"; else -> "generic" }
                val config = WebhookConfig(
                    existing?.id ?: System.currentTimeMillis(), name.text.toString().ifBlank { "Device status" }, platform,
                    url.text.toString().trim(), existing?.enabled ?: true, every.isChecked,
                    battery.isChecked, charging.isChecked, cpu.isChecked, ram.isChecked, time.isChecked
                )
                if (existing == null) store.add(config) else store.update(config)
                refresh()
            }
    ...
    private fun platformName(value: String) = when (value) {
