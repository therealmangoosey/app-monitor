...
class NotificationRulesActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        root.addView(TextView(this).apply { text = "Create alerts for battery, CPU or RAM. Use {value} in a message for the current percentage."; textSize = 15f; setPadding(0, 8, 0, 20) })
    ...
    private fun refresh() {
        ...
        rules.forEach { rule ->
            ...
            buttons.addView(Button(this).apply { text = "Delete"; setOnClickListener { store.remove(rule.id); refresh() } })
    ...
    private fun showRuleDialog(existing: NotificationRule?) {
        ...
        existing?.let { r ->
            metric.setSelection(arrayOf("battery", "cpu", "ram").indexOf(r.metric).coerceAtLeast(0)); operator.setSelection(arrayOf("<=", ">=", "<", ">", "=").indexOf(r.operator).coerceAtLeast(0)); threshold.setText(r.threshold.toString()); title.setText(r.title); message.setText(r.message); importance.setSelection((r.importance - 1).coerceIn(0, 3))
        ...
        AlertDialog.Builder(this).setTitle(if (existing == null) "New notification rule" else "Edit notification rule").setView(box)
            .setPositiveButton("Save") { _, _ ->
                val rule = NotificationRule(existing?.id ?: System.currentTimeMillis(), metric.selectedItem.toString(), operator.selectedItem.toString(), threshold.text.toString().toIntOrNull()?.coerceIn(0, 100) ?: 20, title.text.toString().ifBlank { "App Monitor alert" }, message.text.toString().ifBlank { "{value}%" }, importance.selectedItemPosition + 1, existing?.enabled ?: true)
